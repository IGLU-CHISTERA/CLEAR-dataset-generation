  ...
  while states:
    ...
    if next_node['type'] in qeng.functions_to_be_expanded:
      ...
      for k in filter_option_keys:
        ...
        filter_value_inputs = sorted(next_node['value_inputs'], key=lambda param: template['_param_name_to_attribute'][param])

        if next_node['type'].startswith('relate'):
          param_name = next_node['value_inputs'][0] # First one should be relate   # FIXME : Now that the order of the value inputs doesn't matter, the order of <R> shouldn't either
          filter_value_inputs = sorted(next_node['value_inputs'][1:], key=lambda param: template['_param_name_to_attribute'][param])
